...
export function buildArenaBackdropLayout(
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      ...
      const panelWidth = cellWidth - insetX * 2;
      const panelHeight = cellHeight - insetY * 2;
  ...
  const pipeCount = 4 + Math.floor(random() * 3);
  const pipes: ArenaPipe[] = [];
  for (let index = 0; index < pipeCount; index += 1) {
    const fromLeft = index % 2 === 0;
    const startX = fromLeft ? -24 : width + 24;
    const edgeX = fromLeft ? 95 + random() * 115 : width - 95 - random() * 115;
    const startY = 48 + random() * Math.max(70, height - 170);
    const elbowY = Math.max(28, Math.min(height - 50, startY + (random() - 0.5) * 130));
    const endX = width / 2 + (fromLeft ? -1 : 1) * (105 + random() * 75);
    ...
    pipes.push({
      ...
      thickness: 6 + Math.floor(random() * 5),
      accent: index === 1 || (index === pipeCount - 1 && random() > 0.5) ? 'red' : 'none',
    });
  }
...
function drawPipes(graphics: Phaser.GameObjects.Graphics, pipes: ArenaPipe[]): void {
  for (const pipe of pipes) {
    ...
    if (pipe.accent === 'red') {
      drawPipePath(graphics, pipe.points, 2, PIPE_RED, 0.7);
    }
...
function drawRelayMachine(
  ...
  graphics.fillStyle(0x08090c, 0.92);
  graphics.fillCircle(x, y + 5, radius + 16);

  graphics.fillStyle(MACHINE_DARK, 1);
  graphics.fillCircle(x, y, radius + 10);
  graphics.lineStyle(4, MACHINE_EDGE, 0.45);
  graphics.strokeCircle(x, y, radius + 8);

  graphics.fillStyle(MACHINE_MID, 1);
  graphics.fillCircle(x, y, radius - 2);
  graphics.lineStyle(2, 0x6a6f7d, 0.45);
  graphics.strokeCircle(x, y, radius - 8);

  graphics.fillStyle(MACHINE_RED_DARK, 1);
  graphics.fillCircle(x, y, radius * 0.52);
  graphics.lineStyle(3, MACHINE_RED, 0.72);
  graphics.strokeCircle(x, y, radius * 0.52);

  graphics.fillStyle(MACHINE_RED, 0.46);
  graphics.fillCircle(x - radius * 0.12, y - radius * 0.12, radius * 0.18);

  for (let index = 0; index < 8; index += 1) {
    const angle = (Math.PI * 2 * index) / 8;
    const boltX = x + Math.cos(angle) * (radius + 2);
    const boltY = y + Math.sin(angle) * (radius + 2);
    graphics.fillStyle(0x777b86, 0.75);
    graphics.fillCircle(boltX, boltY, 2);
  }

  graphics.fillStyle(0x17181e, 1);
  graphics.fillRect(x - 74, y + radius + 10, 148, 18);
  graphics.lineStyle(1, 0x474a55, 0.5);
  graphics.strokeRect(x - 74, y + radius + 10, 148, 18);

  graphics.fillStyle(MACHINE_RED, 0.55);
  graphics.fillRect(x - 45, y + radius + 17, 28, 3);
  graphics.fillStyle(0x4a4e58, 0.7);
  graphics.fillRect(x + 10, y + radius + 17, 36, 3);
}
